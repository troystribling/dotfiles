(function (global, factory) {
   ...
   */
  var stringify$3 = stableStringify;
  /**
  ...
  function accessPath(path) {
      return "[" + splitAccessPath(path).map($).join('][') + "]";
  }
  ...
  ***************************************************************************** */
  ...
  (function (message) {
      ...
      function invalidTransformIgnored(transform) {
          return "Ignoring an invalid transform: " + stringify$3(transform) + ".";
      }
      ...
      function projectionOverridden(opt) {
          ...
          return "Layer's shared projection " + stringify$3(parentProjection) + " is overridden by a child projection " + stringify$3(projection) + ".";
      }
      message.projectionOverridden = projectionOverridden;
      function primitiveChannelDef(channel, type, value) {
          return "Channel " + channel + " is a " + type + ". Converted to {value: " + stringify$3(value) + "}.";
      }
      ...
      function emptyFieldDef(fieldDef, channel) {
          return "Dropping " + stringify$3(fieldDef) + " from channel \"" + channel + "\" since it does not contain data field or value.";
      }
      ...
      function unaggregateDomainHasNoEffectForRawField(fieldDef) {
          return "Using unaggregated domain with raw field has no effect (" + stringify$3(fieldDef) + ").";
      }
      ...
      function unaggregatedDomainWithLogScale(fieldDef) {
          return "Unaggregated domain is currently unsupported for log scale (" + stringify$3(fieldDef) + ").";
      }
      ...
      function mergeConflictingProperty(property, propertyOf, v1, v2) {
          return "Conflicting " + propertyOf + " property \"" + property + "\" (" + stringify$3(v1) + " and " + stringify$3(v2) + ").  Using " + stringify$3(v1) + ".";
      }
      ...
      function domainSortDropped(sort) {
          return "Dropping sort property " + stringify$3(sort) + " as unioned domains only support boolean or op 'count'.";
      }
      ...
      function invalidTimeUnit(unitName, value) {
          return "Invalid " + unitName + ": " + stringify$3(value);
      }
      ...
      function droppedDay(d) {
          return "Dropping day from datetime " + stringify$3(d) + " as day cannot be combined with other units.";
      }
  ...
  function vgField(fieldDef, opt) {
      ...
      if (opt.expr) {
          field$$1 = "" + opt.expr + accessPath(field$$1);
      }
      return field$$1;
  }
  ...
  function normalizePathOverlay(spec, config) {
      ...
      if (lineOverlay) {
          layer.push(__assign({}, (projection ? { projection: projection } : {}), { mark: __assign({ type: 'line' }, lineOverlay) }, (selection ? { selection: selection } : {}), { encoding: overlayEncoding }));
      }
      if (pointOverlay) {
          layer.push(__assign({}, (projection ? { projection: projection } : {}), { mark: __assign({ type: 'point', opacity: 1, filled: true }, pointOverlay) }, (selection ? { selection: selection } : {}), { encoding: overlayEncoding }));
      }
   ...
   */
  function timeFormatExpression(field$$1, timeUnit, format, shortTimeLabels, timeFormatConfig, isUTCScale) {
      if (!timeUnit || format) {
          // If there is not time unit, or if user explicitly specify format for axis/legend/text.
          var _format = format || timeFormatConfig; // only use config.timeFormat if there is no timeUnit.
          if (isUTCScale) {
              return "utcFormat(" + field$$1 + ", '" + _format + "')";
          }
          else {
              return "timeFormat(" + field$$1 + ", '" + _format + "')";
          }
      }
      else {
  ...
  function mergeTitleFieldDefs(f1, f2) {
      ...
      f2.forEach(function (fdToMerge) {
          for (var _i = 0, merged_1 = merged; _i < merged_1.length; _i++) {
              ...
              if (stringify$3(fieldDef1) === stringify$3(fdToMerge)) {
                  return;
              }
          }
  ...
  function mergeValuesWithExplicit(v1, v2, property, propertyOf, tieBreaker) {
      ...
      else if (stringify$3(v1.value) === stringify$3(v2.value)) {
          return v1;
      }
      else {
      ...
      if (model.encoding.size || model.markDef.size !== undefined) {
          ...
          if (orient) {
              ...
              if (getFieldDef(model.encoding.size)) {
                  warn(message.cannotUseSizeFieldWithBandSize(channel));
  ...
  function pointPosition(channel, model, defaultRef, vgChannel) {
      ...
      var valueRef = !channelDef && (encoding.latitude || encoding.longitude) ?
          ...
          stackable(channel, encoding[channel], scaleName, scale, stack, getDefaultRef(defaultRef, channel, scaleName, scale, mark));
      return _a = {},
  ...
  function pointPosition2(model, defaultRef, channel) {
      ...
      var valueRef = !channelDef && (encoding.latitude || encoding.longitude) ?
          ...
          { field: model.getName(channel) } :
          stackable2(channel, channelDef, encoding[channel], scaleName, scale, stack, getDefaultRef(defaultRef, baseChannel, scaleName, scale, mark));
  ...
  function assembleLegends(model) {
      ...
      for (var _i = 0, _a = keys(legendComponentIndex); _i < _a.length; _i++) {
          var channel = _a[_i];
          var scaleComponent = model.getScaleComponent(channel);
          var domainHash = stringify$3(scaleComponent.domains);
  ...
  function mergeIfNoConflict(first, second) {
      var allPropertiesShared = every(PROJECTION_PROPERTIES, function (prop) {
          ...
          if (first.explicit.hasOwnProperty(prop) &&
              ...
              stringify$3(first.get(prop)) === stringify$3(second.get(prop))) {
              return true;
          }
          return false;
      });
      var size = stringify$3(first.size) === stringify$3(second.size);
      if (size) {
          if (allPropertiesShared) {
              return first;
          }
          else if (stringify$3(first.explicit) === stringify$3({})) {
              return second;
          }
          else if (stringify$3(second.explicit) === stringify$3({})) {
              return first;
          }
      }
  ...
  function parseExpression(field$$1, parse) {
      var f = "datum" + accessPath(field$$1);
      if (parse === 'number') {
          return "toNumber(" + f + ")";
      }
  ...
  var ParseNode = /** @class */ (function (_super) {
      ...
      ParseNode.make = function (parent, model) {
          ...
          if (data && data.format && data.format.parse) {
              ...
              keys(p_1).forEach(function (field$$1) {
                  parse[field$$1] = p_1[field$$1];
              });
          }
          // We should not parse what has already been parsed in a parent
          var modelParse = model.component.data.ancestorParse;
          keys(modelParse).forEach(function (field$$1) {
              if (parse[field$$1] !== modelParse[field$$1]) {
                  warn(message.differentParse(field$$1, parse[field$$1], modelParse[field$$1]));
              }
              else {
                  delete parse[field$$1];
              }
          });
      ...
      ParseNode.prototype.assembleTransforms = function () {
          var _this = this;
          return keys(this._parse).map(function (field$$1) {
  ...
  function parseSingleChannelDomain(scaleType, domain, model, channel) {
      ...
      if (domain && domain !== 'unaggregated' && !isSelectionDomain(domain)) { // explicit value
          if (fieldDef.bin) {
              warn(message.conflictedDomain(channel));
          }
          else {
  ...
  function zero$1(channel, fieldDef, specifiedScale, markDef) {
      ...
      if (!hasCustomDomain && !fieldDef.bin && contains([X, Y], channel)) {
  ...
  */
  ...
  function values$1(specifiedAxis, model, fieldDef) {
  ...
  function parseAxis(channel, model) {
      ...
      var axisEncode = AXIS_PARTS.reduce(function (e, part) {
          ...
          var value = part === 'labels' ?
              labels$1(model, channel, axisEncoding.labels || {}, axisComponent.get('orient')) :
              axisEncoding[part] || {};
          if (value !== undefined && keys(value).length > 0) {
              e[part] = { update: value };
          }
  ...
  function getProperty$1(property, specifiedAxis, channel, model) {
      ...
      switch (property) {
          ...
          case 'values':
              return values$1(specifiedAxis, model, fieldDef);
      }
      // Otherwise, return specified property.
  ...
  var area = {
      ...
      encodeEntry: function (model) {
          return __assign({}, baseEncodeEntry(model, { size: 'ignore', orient: 'include' }), pointPosition('x', model, 'zeroOrMin'), pointPosition('y', model, 'zeroOrMin'), pointPosition2(model, 'zeroOrMin'), defined(model));
      }
  };
  ...
  function x(model) {
      ...
      if (orient === 'horizontal') {
          return __assign({}, pointPosition('x', model, 'zeroOrMin'), pointPosition2(model, 'zeroOrMin'));
      }
      else { // vertical
          if (isFieldDef(xDef)) {
  ...
  function y(model) {
      ...
      if (orient === 'vertical') {
          return __assign({}, pointPosition('y', model, 'zeroOrMin'), pointPosition2(model, 'zeroOrMin'));
      }
      else {
          if (isFieldDef(yDef)) {
  ...
  var text$3 = {
      ...
      encodeEntry: function (model) {
          ...
          return __assign({}, baseEncodeEntry(model, { size: 'ignore', orient: 'ignore' }), pointPosition('x', model, xDefault(config, textDef)), pointPosition('y', model, mid(height)), text$2(model), nonPosition('size', model, __assign({}, (markDef.size ? { defaultValue: markDef.size } : {}), { vgChannel: 'fontSize' // VL's text size is fontSize
