...
const {sipFile} = require("./utils.js");
	...
	watchBuffer(){
		if(!this.watchingBuffer){
			...
			this.bufferDisposables = new CompositeDisposable(
				...
				this.buffer.onDidDestroy(() => this.unwatchBuffer()),
				this.buffer.onDidDelete(() => this.destroy()),
				this.buffer.onDidReload(() => this.refreshBuffer()),
				this.buffer.onDidSave(() => this.refreshBuffer())
			);
			this.refreshBuffer();
	...
	watchSystem(){
		if(!this.watchingSystem){
			...
			this.systemDisposables = new CompositeDisposable(
				events.onWillThrowWatchError(() => this.destroy()),
				events.onDidDelete(() => this.destroy()),
				events.onDidRename(() => this.setPath(events.getPath())),
				events.onDidChange(() => this.emit("did-change-on-disk"))
			);
		}
	}
