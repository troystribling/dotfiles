...
function renderToNodeStream(code, options = {}) {
  ...
  solidJs.sharedConfig.context.writeResource = (id, p) => {
    ...
    p.then(d => {
      stream$1.push(`<script${options.nonce ? ` nonce="${options.nonce}"` : ""}>_$HYDRATION.resolveResource("${id}", ${JSON.stringify(d).replace(/'/g, "\\'").replace(/\\\"/g, '\\\\\\"')})</script>`);
      ++completed && checkEnd();
    });
  };
...
function renderToWebStream(code, options = {}) {
  ...
  solidJs.sharedConfig.context.writeResource = (id, p) => {
    ...
    p.then(d => {
      writer.write(encoder.encode(`<script${options.nonce ? ` nonce="${options.nonce}"` : ""}>_$HYDRATION.resolveResource("${id}", ${JSON.stringify(d).replace(/'/g, "\\'").replace(/\\\"/g, '\\\\\\"')})</script>`));
      ++completed && checkEnd();
    });
  };
