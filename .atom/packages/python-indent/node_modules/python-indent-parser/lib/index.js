...
var DEDENT_KEYWORDS = ["return", "pass", "break", "continue", "raise"];
function indentationLevel(line) {
    return line.search(/\S|$/);
}
function indentationInfo(lines, tabSize) {
    var parseOutput = parseLines(lines);
    var nextIndent = nextIndentationLevel(parseOutput, lines, tabSize);
    return { nextIndentationLevel: nextIndent, parseOutput: parseOutput };
}
...
function nextIndentationLevel(parseOutput, lines, tabSize) {
    ...
    var dedent = parseOutput.dedent, lastClosedRow = parseOutput.lastClosedRow, lastColonRow = parseOutput.lastColonRow, openBracketStack = parseOutput.openBracketStack;
    ...
    if (!openBracketStack.length) {
        ...
        if (lastClosedRow[1] === row) {
            ...
            var indentLevel = indentationLevel(lines[lastClosedRow[0]]);
            if (lastColonRow === row) {
    ...
    var closedBracketOpenedAfterLineWithCurrentOpen = haveClosedBracket
...
function parseLines(lines) {
        ...
        for (var col = 0; col < lineLength; col += 1) {
            ...
            if (stringDelimiter) {
                ...
                if (c === ":") {
                    ...
                    currentRun = "";
                    var openedRow = openBracketStack.pop()[0];
    ...
    return {
        canHang: canHang, dedent: dedent, lastClosedRow: lastClosedRow, lastColonRow: lastColonRow, openBracketStack: openBracketStack,
    };
}
function shouldHang(line, char) {
    ...
    var allowedChars = "#])}: \t\r".split("");
    var theRest = new Set(line.slice(char).split(""));
    allowedChars.forEach(function (c) { return theRest.delete(c); });
