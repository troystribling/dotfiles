...
for i in range(0, len(steps)):
    sub_steps = steps[i]
    axis.plot(y, kernel_mean[sub_steps[0]], color=color[i], lw="3", zorder=zorder[i], alpha=alpha[0])
    for j in range(1, len(sub_steps)):
        axis.plot(y, kernel_mean[sub_steps[j]], color=color=color[i], lw="3", zorder=zorder[i], alpha=alpha[j])
