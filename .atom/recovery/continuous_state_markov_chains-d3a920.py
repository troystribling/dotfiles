...
x0 = 5.0
αs = [0.1, 0.6, 0.9]
colors = ["#40A6FB", "#36C870", "#E03A29"]
nsample = 10000

figure, axis = pyplot.subplots(figsize=(12, 6))
axis.set_xlabel("Time")
axis.set_ylabel(r"$μ_E$"")
axis.set_title(r"AR(1) $μ_E$")
axis.grid(True, zorder=5)
...
for i in range(0, len(αs)):
    ...
    axis.semilogx(range(0, len(mean)), mean, color=colors[i], label=f"α={α}", lw="2", zorder=10)
    mean = cummean(α, σ, -x0, nsample)
    axis.semilogx(range(0, len(mean)), mean, color=colors[i], lw="2", zorder=10)
axis.legend(bbox_to_anchor=(0.95, 0.9), fontsize=14)
